...
import { Clock, Trophy, Mic } from "lucide-react";
import { useSpeakingSessionsByPart } from "@/hooks/useIeltsAdvancedSpeaking";

export default function SpeakingMyAnswersPage({ params }: { params: { partId: string } }) {
  const { data: sessions, isLoading } = useSpeakingSessionsByPart(params.partId);
  ...
  return (
    <div className="flex flex-col">
      <div className="flex items-center justify-between mb-8 px-4">
        <h2 className="text-2xl font-extrabold text-gray-900 dark:text-white">My Speaking History</h2>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="p-8 text-center bg-gray-50 dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 font-bold text-gray-500 dark:text-slate-400 mx-4">
          You haven&apos;t practiced this speaking part yet.
        </div>
      ) : (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 self-start sm:self-center">
                    {isInProgress && (
                      <Link
                        href={`/ielts/advanced/speaking/${params.partId}?session=${session.id}`}
                    ...
                    {!isInProgress && (
                      <Link
                        href={`/ielts/advanced/speaking/${params.partId}/result/${session.id}`}
                        className="px-5 py-2 border border-gray-200 dark:border-slate-700 rounded-xl text-[14px] font-bold text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors inline-flex items-center gap-2"
                      >
