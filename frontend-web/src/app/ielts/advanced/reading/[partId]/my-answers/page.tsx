  ...
  return (
    <div className="flex flex-col">
       <div className="flex items-center justify-between mb-8 px-4">
         <h2 className="text-2xl font-extrabold text-gray-900 dark:text-white">Answer History</h2>
       ...
       {loading ? (
         ...
         <div className="space-y-4 px-4">
           {history.map((session, idx) => (
             <div key={session.id} className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)] rounded-2xl p-6 hover:shadow-md dark:hover:shadow-[0_8px_30px_rgba(0,0,0,0.4)] transition-shadow">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                   ...
                   {/* Action */}
                   <Link href={`/ielts/advanced/reading/${params.partId}/my-answers/${session.id}`} className="px-5 py-2 border border-gray-200 dark:border-slate-700 rounded-xl text-[14px] font-bold text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors self-start sm:self-center">
                     Details
                   </Link>
                </div>
             </div>
