export function getIeltsBand(score: number): number {
...
export function getIeltsReadingBand(score: number, type: "ACADEMIC" | "GENERAL" = "ACADEMIC"): number {
  if (type === "ACADEMIC") {
    if (score >= 39) return 9.0;
    if (score >= 37) return 8.5;
    if (score >= 35) return 8.0;
    ...
    // General equivalent (mocked, adjust as true scale)
    if (score >= 40) return 9.0;
    if (score >= 39) return 8.5;
    if (score >= 38) return 8.0;
...
export const getIeltsListeningBand = getIeltsBand;
